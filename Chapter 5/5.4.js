...
// Your code here.
function every(array,checkElement){
  var l = array.length;
  for(var i = 0;i<l;i++){
    //console.log(!checkElement(array[i]));
    if(!checkElement(array[i]))return false;
  }
  return true;
}

function some(array,checkElement){
  var l = array.length;
  for(var i = 0;i<l;i++){
    if(checkElement(array[i]))return true;
  }
  return false;
}
function isNaN(el){
  return Object.is(el,NaN);
}
//console.log(NaN===NaN);
